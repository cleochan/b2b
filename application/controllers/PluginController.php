...
if($result)
{
    ...
    function deleteUsersDataAction()
    {
        ...
        $select->where("u.user_id = ?", 1);
        
        $admin_data = $data = $db_info->fetchRow($select);
        $db_info->query('truncate table users');
        $db_info->query('truncate table users_extension');
        ...
        try{
            $user_id = $db_info->insert('users',$data);
        }  catch (Zend_Exception $exp){
            var_dump($exp->getMessage());
        }
        ...
        if($user_id)
        {
            ...
            $user_ext->company = $admin_data['company'];
            $user_ext->contact_name = $admin_data['contact_name'];
            $user_ext->contact_phone = $admin_data['contact_phone'];
            $user_ext->credit = $admin_data['credit'];
            $user_ext->discount = $admin_data['discount'];
            ...
            die("Delete user data log_orders logs_financial data success.");

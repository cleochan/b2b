        ...
        $has=TRUE;
            ...
            do
            {
                ...
                foreach ($product_list_data as $product_data){
                    ...
                    $i  =   1;
                    $images_array   =   array();
                    $image_url      =   $product_data['ProductImages']['CrazySalesProductPictureType'][0]['Path'];
                    $po =   strripos($image_url, '/');
                    $productFilter_model->images_path               =   substr($image_url, 0, $po + 1);
                    if(count($product_data['ProductImages']['CrazySalesProductPictureType']) == 1){
                        $images_array[0]        =   $productFilter_model->getFileName($product_data['ProductImages']['CrazySalesProductPictureType'][0]['Path']); 
                    }else{
                        foreach ($product_data['ProductImages']['CrazySalesProductPictureType'] as $picture_data){
                            if($picture_data['DefalutFlag']==1){
                                $images_array[0]    =   $productFilter_model->getFileName($picture_data['Path']);
                            }else{
                                $images_array[$i]   =   $productFilter_model->getFileName($picture_data['Path']);
                            }
                            $i++;
                        }
                    }
            ...
            if (($handle = fopen($local_order_path, "r")) !== FALSE) {
                ...
                if(!empty($data_array))
                {
                    ...
                    {     
                        ...
                        $getorders_model->shipping_address_1    =   trim($da_val[3]);
                        $getorders_model->shipping_address_2    =   trim($da_val[4]);
                    ...
                    if($day_now && $day_before){
                        ...
                        if($orders_list){
                            foreach ($orders_list as $order){
                                if($product_list_array[$order['supplier_sku'].'-'.$order['final_item_cost']]){
                                    $product_list_array[$order['supplier_sku'].'-'.$order['final_item_cost']]['Quantity']     +=  $order['quantity'];
                                    $product_list_array[$order['supplier_sku'].'-'.$order['final_item_cost']]['Price']        =   $order['final_item_cost'];
                                    $product_list_array[$order['supplier_sku'].'-'.$order['final_item_cost']]['ship_cost']    +=  $order['final_ship_cost'];
                                }else{
                                    $product_list_array[$order['supplier_sku'].'-'.$order['final_item_cost']]['ItemCode'] =   $order['supplier_sku'];
                                    $product_list_array[$order['supplier_sku'].'-'.$order['final_item_cost']]['Quantity'] =   $order['quantity'];
                                    $product_list_array[$order['supplier_sku'].'-'.$order['final_item_cost']]['Price']    =   $order['final_item_cost'];
                                    $product_list_array[$order['supplier_sku'].'-'.$order['final_item_cost']]['ship_cost']=   $order['final_ship_cost'];
                                }
